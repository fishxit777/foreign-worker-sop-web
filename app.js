const scenarioData = {
  overtime: {
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `補充：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，語氣清楚、禮貌、不要太像命令。`
    ]
  },
  leave: {
    label: "請假規則",
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `補充：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，讓外籍員工可以清楚理解請假流程。`
    ]
  },
  safety: {
    label: "工安提醒",
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `今日重點：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，語氣要像現場主管提醒，不要像法規文件。`
    ]
  },
  machine: {
    label: "機台 SOP",
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `本次操作重點：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，保留 SOP 條列格式，讓現場容易照做。`
    ]
  },
  dorm: {
    label: "宿舍規定",
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `補充規定：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，語氣清楚但不要讓員工覺得被責罵。`
    ]
  },
  line: {
    label: "LINE 群組規則",
    ...
    build: ({ company, date, time, place, owner, note, language }) => [
      ...
      note ? `群組規則：${note}` : "",
      "",
      `請用萬語通翻譯成${language}，保留 LINE 群組公告的口語感。`
    ]
  }
};
...
function downloadOutput() {
  ...
  link.href = url;
  link.download = `foreign-worker-sop-${today}.txt`;
